...
class UniswapV3HedgeCalculator(tk.Tk):
    ...
    def calculate_dynamic_hedge(self):
        ...
        try:
            ...
            for step_idx, price in enumerate(prices_sequence[1:], start=1):
                ...
                # Новый хедж
                hedge_eth = prev_hedge + hedge_change
                ...
                # P&L рассчитывается по-разному для шорта и лонга
                # Для шорта (отрицательное количество ETH):
                # - Если цена падает, это прибыль
                # - Если цена растет, это убыток
                
                # Расчет P&L хеджа для этого шага
                # Для оставшейся части позиции
                unrealized_pnl = prev_hedge * (prev_price - price)
                
                # Для измененной части позиции (если закрываем часть позиции)
                realized_pnl = 0
                if abs(hedge_change) > 0:
                    # Если уменьшаем шорт (закрываем часть позиции)
                    if hedge_change > 0 and prev_hedge < 0:
                        # Мы купили обратно часть шорта
                        realized_pnl = hedge_change * (prev_price - price)
                    # Если увеличиваем шорт (открываем новую позицию)
                    # P&L = 0, так как мы только что открыли позицию
    ...
    def display_dynamic_results(self):
        ...
        # Заголовок
        header = "Шаг\tЦена\t\tETH в пуле\tФьючерс\tДельта\tИзменение\tКомиссия\tP&L\tНакопл. P&L\n"
        self.dynamic_results_text.insert(tk.END, header)
        self.dynamic_results_text.insert(tk.END, "-" * 120 + "\n")
        ...
        for result in self.dynamic_results:
            ...
            row = (f"{result['step']}\t{result['price']:.2f} {price_indicator}\t" +
                  ...
                  f"{result['fee']:.2f}\t{result['hedge_pnl']:.2f}\t{result['cumulative_pnl']:.2f}\n")
    ...
    def plot_dynamic_results(self):
        ...
        initial_value = initial_eth * initial_price
        ...
        for i, result in enumerate(self.dynamic_results):
            ...
            # P&L основной позиции: как изменилась стоимость ETH в пуле
            # Текущая стоимость = количество ETH * текущая цена
            # P&L = текущая стоимость - начальная стоимость
            current_value = pool_eth * price
            base_pnl_value = current_value - initial_value
        ...
        for i, step in enumerate(steps):
            ...
            if i > 0 and abs(self.dynamic_results[i]['hedge_change']) > 0.01:
                ...
                marker = "△" if change > 0 else "▽"  # Треугольник вверх/вниз
        ...
        self.dynamic_ax.set_title('Профиль P&L', fontsize=12, fontweight='bold')
        ...
        # Добавляем пояснение к маркерам
        self.dynamic_ax.annotate("△ - уменьшение шорта, ▽ - увеличение шорта", 
                              xy=(0.5, 0.01),
