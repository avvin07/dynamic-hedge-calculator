...
import traceback
import csv
from logic import parse_float, calculate_liquidity, position_amounts, calculate_hedge_fee, HedgeTransaction
from plotting import plot_liquidity_range, plot_hedged_position, plot_hedged_position_with_exit
...
# Класс с поддержкой placeholder и подсветкой при пустом вводе
...
class UniswapV3HedgeCalculator(tk.Tk):
    def __init__(self):
        super().__init__()
        # Регистрируем проверку для числовых полей
        self.vcmd = (self.register(self._validate_numeric), '%P')
    ...
    def _validate_numeric(self, new_val):
        """Validate entry as float allowing ',' or '.' separators."""
        if new_val == '':
            return True
        try:
            parse_float(new_val)
    ...
    def create_main_tab_widgets(self):
        ...
        PlaceholderEntry(input_frame, textvariable=self.current_price, width=15,
                         placeholder="1616", validate='key', validatecommand=self.vcmd).grid(row=0, column=1, pady=5)
        
        # Lower bound
        ttk.Label(input_frame, text="Нижняя граница (USDC):").grid(row=1, column=0, sticky="w", pady=5)
        PlaceholderEntry(input_frame, textvariable=self.lower_bound, width=15,
                         placeholder="1550", validate='key', validatecommand=self.vcmd).grid(row=1, column=1, pady=5)
        
        # Upper bound
        ttk.Label(input_frame, text="Верхняя граница (USDC):").grid(row=2, column=0, sticky="w", pady=5)
        PlaceholderEntry(input_frame, textvariable=self.upper_bound, width=15,
                         placeholder="2200", validate='key', validatecommand=self.vcmd).grid(row=2, column=1, pady=5)
        
        # Total pool value
        ttk.Label(input_frame, text="Общая стоимость пула (USDC):").grid(row=0, column=2, sticky="w", padx=20, pady=5)
        PlaceholderEntry(input_frame, textvariable=self.total_pool_value, width=15,
                         placeholder="10000", validate='key', validatecommand=self.vcmd).grid(row=0, column=3, pady=5)
        
        # Автоматически скрываем возможные кнопки в колонке 2 (Экспорт/Загрузка)
        for widget in input_frame.grid_slaves(row=1, column=2) + input_frame.grid_slaves(row=2, column=2):
            widget.grid_forget()
        ...
        # Расширяем на 6 колонок для трёх блоков: текущая, нижняя и верхняя границы
        for col in range(6):
        ...
        ttk.Label(self.results_frame, textvariable=self.liquidity_var, font=('Helvetica', 10, 'bold')).grid(row=1, column=1, sticky="w", pady=5)
        ttk.Label(self.results_frame, text="Количество ETH:").grid(row=2, column=0, sticky="w", pady=5)
        ttk.Label(self.results_frame, textvariable=self.eth_amount_var, font=('Helvetica', 10, 'bold')).grid(row=2, column=1, sticky="w", pady=5)
        ttk.Label(self.results_frame, text="Количество USDC:").grid(row=3, column=0, sticky="w", pady=5)
        ttk.Label(self.results_frame, textvariable=self.usdc_amount_var, font=('Helvetica', 10, 'bold')).grid(row=3, column=1, sticky="w", pady=5)
        
        # --- При достижении нижней границы (центр) ---
        ttk.Label(self.results_frame, text="При достижении нижней границы:").grid(row=0, column=2, sticky="ew", pady=5, columnspan=2)
        ttk.Label(self.results_frame, text="Количество ETH:").grid(row=1, column=2, sticky="w", pady=5)
        ttk.Label(self.results_frame, textvariable=self.lower_eth_var, font=('Helvetica', 10, 'bold')).grid(row=1, column=3, sticky="w", pady=5)
        ttk.Label(self.results_frame, text="Количество USDC:").grid(row=2, column=2, sticky="w", pady=5)
        ttk.Label(self.results_frame, textvariable=self.lower_usdc_var, font=('Helvetica', 10, 'bold')).grid(row=2, column=3, sticky="w", pady=5)
        
        # --- При достижении верхней границы (справа) ---
        ttk.Label(self.results_frame, text="При достижении верхней границы:").grid(row=0, column=4, sticky="ew", pady=5, columnspan=2)
        ttk.Label(self.results_frame, text="Количество ETH:").grid(row=1, column=4, sticky="w", pady=5)
        ttk.Label(self.results_frame, textvariable=self.upper_eth_var, font=('Helvetica', 10, 'bold')).grid(row=1, column=5, sticky="w", pady=5)
        ttk.Label(self.results_frame, text="Количество USDC:").grid(row=2, column=4, sticky="w", pady=5)
        ttk.Label(self.results_frame, textvariable=self.upper_usdc_var, font=('Helvetica', 10, 'bold')).grid(row=2, column=5, sticky="w", pady=5)
        
        # Добавляем кнопку "Обновить графики" под блоками результатов (центр)
        update_button = ttk.Button(self.results_frame, text="Обновить графики", command=self.refresh_plots, width=20)
        update_button.grid(row=3, column=2, columnspan=2, pady=10, sticky="nsew")
    ...
    def create_hedge_tab_widgets(self):
        ...
        self.hedge_amount_entry = PlaceholderEntry(hedge_frame, textvariable=self.hedge_amount, width=15,
                                                 placeholder=str(self.hedge_amount.get()),
        ...
        self.hedge_price_entry = PlaceholderEntry(hedge_frame, textvariable=self.hedge_price, width=15,
                                                 placeholder=str(self.hedge_price.get()),
                                                 validate='key', validatecommand=self.vcmd)
        self.hedge_price_entry.grid(row=3, column=1, pady=5)
        
        # Hedge fee - добавляем поле для комиссии
        ttk.Label(hedge_frame, text="Комиссия (%):").grid(row=4, column=0, sticky="w", pady=5)
        self.hedge_fee_entry = PlaceholderEntry(hedge_frame, textvariable=self.hedge_fee_percent, width=15,
                                                placeholder=str(self.hedge_fee_percent.get()),
                                                validate='key', validatecommand=self.vcmd)
        self.hedge_fee_entry.grid(row=4, column=1, pady=5)
        ...
        PlaceholderEntry(exit_frame, textvariable=self.exit_price, width=15,
                         placeholder=str(self.exit_price.get()),
                         validate='key', validatecommand=self.vcmd).grid(row=0, column=1, pady=5)
    ...
    def create_grid_tab_widgets(self):
        ...
        self.grid_step = PlaceholderEntry(settings_frame, width=15,
                                         placeholder="100",
                                         validate='key', validatecommand=self.vcmd)
        ...
        self.grid_fee = PlaceholderEntry(settings_frame, width=15,
                                        placeholder="0.2",
                                        validate='key', validatecommand=self.vcmd)
    ...
    def auto_hedge(self):
        ...
        try:
            # Парсим параметры
            current_price = parse_float(str(self.current_price.get()))
            lower_bound = parse_float(str(self.lower_bound.get()))
            upper_bound = parse_float(str(self.upper_bound.get()))
            total_pool_value = parse_float(str(self.total_pool_value.get()))
            # Расчет ликвидности и ETH через logic
            liquidity = calculate_liquidity(current_price, lower_bound, upper_bound, total_pool_value)
            eth_amount, _ = position_amounts(liquidity, current_price, lower_bound, upper_bound)
            # Устанавливаем хедж
    ...
    def calculate_only_text(self):
        ...
        try:
            ...
            eth_amount, usdc_amount = position_amounts(liquidity, current_price, lower_bound, upper_bound)
            ...
            self.liquidity_var.set(f"{liquidity:.2f}")
            self.eth_amount_var.set(f"{eth_amount:.2f}")
            self.usdc_amount_var.set(f"{usdc_amount:.2f}")

            # Обновляем результаты при достижении верхней и нижней границы
            self.upper_eth_var.set(f"{upper_eth_amount:.2f}")
            self.upper_usdc_var.set(f"{upper_usdc_amount:.2f}")
            self.lower_eth_var.set(f"{lower_eth_amount:.2f}")
            self.lower_usdc_var.set(f"{lower_usdc_amount:.2f}")
            ...
            if self.hedge_enabled.get():
                hedge_amount = parse_float(str(self.hedge_amount.get()))
                hedge_price = parse_float(str(self.hedge_price.get()))
                
                # Расчет комиссии через logic
                fee_percent = parse_float(str(self.hedge_fee_percent.get()))
                hedge_fee = calculate_hedge_fee(hedge_amount, hedge_price, fee_percent)
    ...
    def calculate(self):
        ...
        try:
            ...
            # Plot main position
            plot_liquidity_range(self.ax1, self.ax2,
                                 current_price, lower_bound, upper_bound, total_pool_value)

            # Plot hedged position / exit
            try:
                exit_price = parse_float(str(self.exit_price.get()))
                fee_percent = parse_float(str(self.hedge_fee_percent.get()))
                if exit_price > 0:
                    plot_hedged_position_with_exit(
                        self.hedge_ax2,
                        current_price, lower_bound, upper_bound,
                        total_pool_value, hedge_amount, hedge_price,
                        fee_percent, exit_price
                    )
                else:
                    plot_hedged_position(
                        self.hedge_ax2,
                        current_price, lower_bound, upper_bound,
                        total_pool_value, hedge_amount, hedge_price,
                        fee_percent
                ...
                print(f"Error plotting hedged: {e}")
    ...
    def calculate_exit_results_no_graph(self):
        ...
        try:
            ...
            # Рассчитываем ликвидность через utility
            liquidity = calculate_liquidity(current_price, lower_bound, upper_bound, total_pool_value)
 
            # Позиция на текущей цене
            eth_amount_current, usdc_amount_current = position_amounts(liquidity, current_price, lower_bound, upper_bound)
            current_total_value = eth_amount_current * current_price + usdc_amount_current
 
            # Позиция на выходе
            eth_amount_exit, usdc_amount_exit = position_amounts(liquidity, exit_price, lower_bound, upper_bound)
            ...
            # Расчет P&L и комиссии хеджа на выходе
            hedge_pnl = 0
            hedge_fee = 0
            if self.hedge_enabled.get():
                hedge_amount = parse_float(str(self.hedge_amount.get()))
                ...
                fee_percent = parse_float(str(self.hedge_fee_percent.get()))
                hedge_fee = calculate_hedge_fee(hedge_amount, hedge_price, fee_percent)
                self.exit_fee_value.set(f"{hedge_fee:.2f}")
                # P&L шорта
    ...
    def calculate_delta_for_price(self, price):
        ...
        try:
            ...
            upper_bound = parse_float(str(self.upper_bound.get()))
            total_pool_value = parse_float(str(self.total_pool_value.get()))
            # Рассчитываем ликвидность и возвращаем объем ETH
            liquidity = calculate_liquidity(current_price, lower_bound, upper_bound, total_pool_value)
            eth_amount, _ = position_amounts(liquidity, price, lower_bound, upper_bound)
            return eth_amount
        except Exception as e:
            print(f"Ошибка при расчете дельты: {e}")
            return 0
    ...
    def create_dynamic_tab_widgets(self):
        ...
        PlaceholderEntry(settings_frame, textvariable=self.dynamic_step, width=10,
                         placeholder=str(self.dynamic_step.get()),
                         validate='key', validatecommand=self.vcmd).grid(row=0, column=1, padx=5, pady=5, sticky="w")
        
        # Комиссия
        ttk.Label(settings_frame, text="Комиссия (%):").grid(row=0, column=2, padx=5, pady=5, sticky="w")
        PlaceholderEntry(settings_frame, textvariable=self.dynamic_fee, width=10,
                         placeholder=str(self.dynamic_fee.get()),
                         validate='key', validatecommand=self.vcmd).grid(row=0, column=3, padx=5, pady=5, sticky="w")
    ...
    def create_price_field(self, index, price_var):
        ...
        entry = PlaceholderEntry(frame, textvariable=price_var, width=10,
                                 placeholder=str(price_var.get()),
                                 validate='key', validatecommand=self.vcmd)
    ...
    def form_price_sequence(self, entry_price, user_prices):
        ...
        """
        try:
            step = parse_float(str(self.dynamic_step.get()))
            ...
            ordered_prices = [parse_float(str(var.get())) for var in self.dynamic_price_vars]
    ...
    def plot_dynamic_results(self):
        ...
        try:
            # Просто очищаем ось и переиспользуем canvas
            self.dynamic_ax.clear()
            # Обновляем отступы (если изменились параметры)
    ...
    def save_main_plot(self):
        """Сохраняет основной график в PNG"""
    ...
    def save_hedge_plot(self):
        """Сохраняет хедж-график в PNG"""
        file_path = filedialog.asksaveasfilename(defaultextension='.png',
                                                 filetypes=[('PNG files', '*.png'), ('All files', '*.*')],
                                                 title='Сохранить хедж-график')
        if file_path:
            self.hedge_figure.savefig(file_path, dpi=150)
